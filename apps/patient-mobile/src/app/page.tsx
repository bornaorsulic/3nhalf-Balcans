...
import { Card, Delta, LinkCard, LoadingCards, SectionTitle, StatusPill } from "@/components/ui";
import { API_MODE } from "@/config/app";
import { useAppointmentQuestions, useDiary, useProfile, useSummaries, useWearables } from "@/lib/api/hooks";
...
function CheckInCard() {
  ...
  if (today) {
    return (
      <Card className="flex items-center gap-3">
        ...
        <Link href="/log" className="text-sm font-semibold text-primary">
          Edit
        </Link>
      </Card>
    );
  ...
  return (
    <Link href="/log" className="block rounded-card bg-primary p-4 text-on-primary shadow-card transition-colors hover:bg-primary-strong">
...
function TrendTiles() {
  ...
  return (
    <>
      <SectionTitle
        action={
          <Link href="/health" className="text-xs font-semibold text-primary">
            See all
          </Link>
        }
      >
      ...
      {data ? (
        <div className="grid grid-cols-2 gap-3">
          {tiles.map((metric) => {
            ...
            return (
              <Link key={metric} href="/health" className="rounded-card bg-surface p-3.5 shadow-card">
