# Patient app ↔ backend API contract

For Person 1 (AI / backend) and Person 2 (data / RAG). The patient app runs on mock data until the
backend implements these endpoints; then set `NEXT_PUBLIC_API_MODE=http`.

**Source of truth for every shape: [`src/lib/api/types.ts`](src/lib/api/types.ts).** The mock
implementation ([`src/lib/api/mock/`](src/lib/api/mock)) returns the same shapes, so it doubles as
example data.

- Base URL: `NEXT_PUBLIC_API_BASE_URL` (default `http://localhost:8000/api/v1`)
- All routes are scoped to one patient: `/patients/{patientId}` (default id `demo-patient`)
...
- `safety.level`: `urgent` shows a red banner with a call-112 button; `caution` shows an amber note.
  The app also runs its own red-flag check (`src/lib/safety.ts`), but the backend must triage too.
...
`ClinicianSummary.status` is `in_review` or `approved`. The app only shows `body` for approved
summaries; patients never see unapproved AI output. The clinician dashboard (Person 3) is where
summaries get approved.
