...
TABLES: list[tuple[str, str]] = [
    ...
    (
        ...
        # "every Tuesday 09:00-12:00". Concrete bookable slots are generated from
        # these rules for the next few weeks (see backend/schedule.py).
        "availability_rules",
        """
...
ALTERS = [
    ...
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS time_format VARCHAR(8) DEFAULT '24h';",
    # Where a slot came from: generated by the weekly template, or added by hand.
    "ALTER TABLE availability_slots ADD COLUMN IF NOT EXISTS source VARCHAR(20) DEFAULT 'manual';",
    # Uploaded files: blood work PDFs, images and other documents added by either side.
    # Voice notes and cached text-to-speech are stored as private file rows too.
    "ALTER TABLE files ADD COLUMN IF NOT EXISTS content BYTEA;",
    "ALTER TABLE files ADD COLUMN IF NOT EXISTS purpose VARCHAR(40);",
...
INDEXES = [
    ...
    "CREATE INDEX IF NOT EXISTS rules_clinician_idx ON availability_rules (clinician_id, weekday);",
    "CREATE INDEX IF NOT EXISTS files_patient_idx ON files (patient_id, created_at DESC);",
